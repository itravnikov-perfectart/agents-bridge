...
export function ChatTabs({ selectedAgent }: ChatTabsProps) {
  ...
  const { getActiveTaskIds, getTaskHistory, getTaskDetails, resumeTask, onLoadingStateChange } = useWebSocketConnection();
  ...
  useEffect(() => {
    if (view === 'active' && activeTasks.length > 0) {
      ...
      if (resumedTaskIdRef.current) {
        const resumedTaskIndex = activeTasks.findIndex((t: any) => t.id === resumedTaskIdRef.current);
